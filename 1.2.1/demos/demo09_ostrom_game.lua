...
logger = Agent{
	execute = function(ag)
		...
		if (randomObj:number() < P_DEFOREST) and (#ag.forestNeighs > 0) then  
      local index = randomObj:integer(1,#ag.forestNeighs)
			local neigh = ag.forestNeighs[index]
			neigh.agent = ag
      ...
      local index = 0
        ...
        local index = randomObj:integer(1,#ag.deforestedNeighs)
				local neigh = ag.deforestedNeighs[index]
				ag:move(neigh)
			end
...
fiscal = Agent{
	...
	execute = function(agent)
		...
		forEachCell(it, function(cell)
			...
			if (randomObj:number() < P_ENFORCEMENT) then
				if randomObj:number() < P_CAUGHT then
