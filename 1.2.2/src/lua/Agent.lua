...
local deadAgentMetaTable_ = {__index = function()
	error("Error: Trying to use a function or an attribute of a dead Agent.", 2)
end}
...
Agent_ = {
	...
	die = function(self, remove_placements)
		if remove_placements == true and not self.parent then
			error("Error: Cannot remove the placements of an Agent that do not belong to a Society.", 2)
		end
    ...
    if(not event) then
      error("Error: Event expected, got nil.", 2)
    end
	...
	getCell = function(agent, placement)
		...
		if agent[placement] == nil or type(agent[placement]) ~= "Trajectory" then
			if placement == "placement" then
				...
				error("Error: Placement '".. placement.. "' not found.", 2)
			end
		end
		...
		if data.delay == nil or data.delay == 0 then
			if data.subject then
				...
				if type(data.receiver[call]) ~= "function" then
					error("Error: Receiver (id = "..data.receiver:getID()..") does not implement function "..call..".", 2)
				end
