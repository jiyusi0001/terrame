...
CellularSpace_ = {
	...
	loadNeighborhood = function(self, tbAttrLoad)
		if type(tbAttrLoad) ~= "table" then
			error("Argument should be a table, got" .. type(tbAttrLoad))
		end
...
function CellularSpace(attrTab)
	...
	if(attrTab.database ~= nil and attrTab.autoload == nil) then
		attrTab.autoload = true
	end
	...
	if attrTab.xdim ~= nil then -- rectangular "virtual" cellular space
		...
		if attrTab.database:endswith(".shp") then
	...
	if(attrTab.xdim) then
		attrTab:load()
	else	
		if(attrTab.database ~= nil and attrTab.autoload) then
			attrTab:load()
			-- needed for Environment's loadNeighborhood	
			attrTab.layer = attrTab.cObj_:getLayerName()
		end
	end
	return attrTab
end
