...
ObserverUDPSender::ObserverUDPSender(Subject *sub) 
    : QThread(), ObserverInterf(sub)
{
    init();
}
...
void ObserverUDPSender::init()
{
    ...
    udpGUI = new ObserverUdpSenderGUI();
...
bool ObserverUDPSender::draw(QDataStream &in)
{
#ifdef TME_STATISTIC
...
void ObserverUDPSender::setAttributes(QStringList& attrs)
{
    attributes = attrs;
...
QStringList& ObserverUDPSender::getParameters()
{
    return parameters;
}

void ObserverUDPSender::setParameters(QStringList& params)
{
    parameters = params;
}
...
void ObserverUDPSender::setCompressDatagram(bool compress)
{
    compressDatagram = compress;
