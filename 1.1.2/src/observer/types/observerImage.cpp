...
#include "image/observerImageGUI.h"
...
{
    obsImgGUI = new ObserverImageGUI();
...
bool ObserverImage::draw(QDataStream &in)
{
    bool decoded = false;
    QString msg;
    in >> msg;
...
void ObserverImage::setHeaders(QStringList headers, QStringList legKeys, QStringList legAttrib)
{
    ...
    for( int i = 0; i < itemList.size(); i++)
    {
        if ((! mapAttributes->contains(itemList.at(i)) )
                && (itemList.at(i) != QString("x"))
                && (itemList.at(i) != QString("y")) )
        {
            obsAttrib.append(itemList.at(i));
            attrib = new Attributes(itemList.at(i), width * height,
            ...
            if ((legKeys.size() > 0) && ( contAttrib + 1 <= numLegs))
            {
                ...
                ObserverMap::createColorsBar(legAttrib.at(colorBar + cont),
