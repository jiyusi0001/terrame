...
void ObserverImpl::setVisible(bool b)
{
    ObserverImpl::visible = b;
}
...
void ObserverImpl::setObsHandle(Observer* o)
{
    obsHandle_	= o;
    subject_->attach(o);
}
...
Observer * SubjectImpl::getObserverById(int observerId)
{
    Observer *obs = 0;

    for (ObsListIterator i (observers.begin()); i != observers.end(); ++i)
    {
        if ( (*i)->getId() == observerId)
        {
            obs = *i;
            break;
        }
