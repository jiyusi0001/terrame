...
int luaTrajectory::clear( lua_State* L)
{
    luaRegion::clear();
    return 0;
}
...
int luaTrajectory::createObserver( lua_State *L )
{
    ...
    {
        QStringList allAttribs, obsAttribs, obsParams;
        ...
        if (obsUDPSender)
        {
            ...
            //if (obsParams.at(0).isEmpty())
            if (obsParams.isEmpty())
            {
                if (! QUIET_MODE )
            ...
            {
                obsUDPSender->setPort(obsParams.at(0).toInt());
            }

            // broadcast
            if ((obsParams.size() == 1)
                || ((obsParams.size() == 2) && obsParams.at(1).isEmpty()) )
            {
                if (! QUIET_MODE )
            ...
            else
            {
                ...
                for(int i = 1; i < obsParams.size(); i++){
                    if (! obsParams.at(i).isEmpty())
                        obsUDPSender->addHost(obsParams.at(i));
                }
            }
    ...
    else
    {
        ...
        if (typeObserver == TObsMap)
        {
            ...
            obsMap->setHeaders(obsAttribs, obsParams, obsParamsAtribs);
