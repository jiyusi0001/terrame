...
ObserverLogFile::ObserverLogFile(Subject *s) : QObject(), ObserverInterf( s ) // , QThread()
{
    init();
}
...
bool ObserverLogFile::draw(QDataStream &in)
{
    QString msg;
    in >> msg;
...
void ObserverLogFile::setHeaders(QStringList h)
{
    itemList = h;
