...
bool ObserverScheduler::draw(QDataStream & in)
{
        ...
        switch (typeOfData)
        {
            ...
            case (TObsNumber):
                if (key.contains("@"))
                {
                    ...
                    item->setText(Time,  number2QString(num));
                    j += 3;

                    // recupera o period
                    num = tokens.at(j).toDouble();
                    item->setText(Periodicity,  number2QString(num));
                    j += 3;

                    // recupera o priority
                    num = tokens.at(j).toDouble();
                    item->setText(Priority, number2QString(num));
...
const QString ObserverScheduler::number2QString(double number)
{
    static QString complement("000000");
