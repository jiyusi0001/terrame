...
static const QString COLORBAR_SEP= "|";
...
bool ObserverMap::draw(QDataStream &in)
{
    bool decoded = false;
    QString msg;
    in >> msg;
...
void ObserverMap::setHeaders(QStringList headers, QStringList legKeys, QStringList legAttrib)
{
    connectTreeLayer(false);
    ...
    // são 11 itens de legenda para cada atributo observado
    ...
    for( int i = 0; i < itemList.size(); i++)
    {
        if ((! mapAttributes->contains(itemList.at(i)))
            && (itemList.at(i) != QString("x")) && (itemList.at(i) != QString("y")) )
        {
            ...
            {
                attrib->setDataType( (TypesOfData) legAttrib.at(type + cont).toInt());
                attrib->setGroupMode( (GroupingMode) legAttrib.at(mode + cont).toInt());
                attrib->setSlices(legAttrib.at(slices + cont).toInt() - 1);				// conta com o zero
                attrib->setPrecisionNumber(legAttrib.at(precision + cont).toInt() - 1);	// conta com o zero
                attrib->setStdDeviation( (StdDev) legAttrib.at(stdDeviation + cont).toInt());
                attrib->setMaxValue(legAttrib.at(max + cont).toDouble());
                attrib->setMinValue(legAttrib.at(min + cont).toDouble());

                //Fonte
                attrib->setFontFamily(legAttrib.at(font + cont));
                attrib->setFontSize(legAttrib.at(fontSize + cont).toInt());

                //Converte o código ASCII do símbolo em caracter
                bool ok = false;
                int asciiCode = legAttrib.at(symbol + cont).toInt(&ok, 10);
                if (ok)
                    attrib->setSymbol( QString( QChar(asciiCode ) ));
                else
                    attrib->setSymbol(legAttrib.at(symbol + cont));
                
                std::vector<ColorBar> colorBarVec;
                std::vector<ColorBar> stdColorBarVec;
                QStringList labelList, valueList;

                createColorsBar(legAttrib.at(colorBar + cont),
...
void ObserverMap::zoomChanged(QRect zoomRect, double factWidth, double factHeight)
{
    double zoom = 0.0;
    ...
    if (factWidth < factHeight)
        ...
        zoom = factHeight - 0.01;

#ifdef DEBUG_OBSERVER
    qDebug() << "zoomChanged: " << zoom;
    qDebug() << "factWidth: " << factWidth << " factHeight: " << factHeight;
...
void ObserverMap::connectTreeLayer(bool connect)
{
    ...
    std::vector<ColorBar> &stdColorBarVec, QStringList &valueList, QStringList &labelList)
{
    ...
    int pos = colors.indexOf(COLORBAR_SEP); // separa a colorBar e o stdColorBar
