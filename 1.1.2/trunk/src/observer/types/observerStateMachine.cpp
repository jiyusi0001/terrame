...
ObserverStateMachine::ObserverStateMachine(Subject *sub, QWidget *parent)
...
bool ObserverStateMachine::draw(QDataStream &in)
{
    QString msg;
    in >> msg;
    ...
    for (int i=0; i < qtdParametros; i++)
    {
        ...
        bool contains = attributes.contains(key);
...
void ObserverStateMachine::setAttributes(QStringList attribs, QStringList legKeys, QStringList legAttrib)
{
    attributes = attribs;
    ...
    for( int i = 0; i < attribs.size(); i++)
    {
            ...
            && (attribs.at(i) != QString("x")) && (attribs.at(i) != QString("y")) )
        {
            ...
            if (legKeys.size() > 0)
            {
                attrib->setDataType( (TypesOfData) legAttrib.at(type).toInt());
                attrib->setGroupMode( (GroupingMode) legAttrib.at(mode ).toInt());
                attrib->setSlices(legAttrib.at(slices).toInt() - 1);				// conta com o zero
                attrib->setPrecisionNumber(legAttrib.at(precision).toInt() - 1);	// conta com o zero
                attrib->setStdDeviation( (StdDev) legAttrib.at(stdDeviation ).toInt());
                attrib->setMaxValue(legAttrib.at(max).toDouble());
                attrib->setMinValue(legAttrib.at(min).toDouble());

                //Fonte
                attrib->setFontFamily(legAttrib.at(font));
                attrib->setFontSize(legAttrib.at(fontSize).toInt());

                //Converte o código ASCII do símbolo em caracter
                bool ok = false;
                int asciiCode = legAttrib.at(symbol).toInt(&ok, 10);
                if (ok)
                    attrib->setSymbol( QString( QChar(asciiCode) ));
                else
                    attrib->setSymbol(legAttrib.at(symbol));

                std::vector<ColorBar> colorBarVec;
                std::vector<ColorBar> stdColorBarVec;
                QStringList labelList, valueList;

                ObserverMap::createColorsBar(legAttrib.at(colorBar),
...
void ObserverStateMachine::addState(QList<QPair<QString, QString> > &allStates)
{
    ...
    int resizeHeight = (allStates.size() % 2 == 0) ? allStates.size() : allStates.size() - 1;
...
void ObserverStateMachine::zoomChanged(const QRectF &zoomRect, float factWidth, float factHeight)
{
    ...
    qDebug() << "zoomRect: " << zoomRect << " factWidth: "  << factWidth 
...
int ObserverStateMachine::calculeZoom(bool in)
{
